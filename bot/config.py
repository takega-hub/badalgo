...
class AppSettings:
    ...
    kline_limit: int = 500       # сколько свечей тянуть для сигналов (для ML нужно минимум 200)
    ...
    ml_confidence_threshold: float = 0.6  # Минимальная уверенность ML модели для открытия позиции (0-1)
    ml_min_signal_strength: str = "умеренное"  # Минимальная сила сигнала: "слабое", "умеренное", "среднее", "сильное", "очень_сильное"
    ...
    enable_trend_strategy: bool = True  # Трендовая стратегия
    enable_flat_strategy: bool = True   # Флэтовая стратегия
    enable_ml_strategy: bool = False    # ML стратегия
    # Приоритетная стратегия при конфликте сигналов
    strategy_priority: str = "trend"  # "trend" (TREND/FLAT приоритет), "ml" (ML приоритет), "hybrid" (обе равноправны)
...
def load_settings() -> AppSettings:
    ...
    if strategy_priority in ("trend", "ml", "hybrid"):
        settings.strategy_priority = strategy_priority
        print(f"[config] STRATEGY_PRIORITY loaded from .env: {settings.strategy_priority}")
    else:
        print(f"[config] No STRATEGY_PRIORITY in .env, using default: {settings.strategy_priority}")
    ...
    if settings.symbol:
        ...
        if models_dir.exists():
            ...
            if not current_model_matches:
                found_model = None
                for model_file in models_dir.glob(f"*_{settings.symbol}_*.pkl"):
    ...
    # Если модель не задана через env, проверяем наличие модели и используем её
    # НО НЕ перезаписываем настройки, если они уже заданы в .env
    if not ml_strategy_type and not ml_model_path:
        # Проверяем наличие модели Random Forest (лучшая модель)
        # Используем модель для ТЕКУЩЕГО символа, а не принудительно ETHUSDT
        if settings.symbol:
            # Ищем модель для текущего символа
            model_path = pathlib.Path(__file__).parent.parent / "ml_models" / f"rf_{settings.symbol}_15.pkl"
            if model_path.exists():
                print(f"[config] Found ML model for {settings.symbol}, enabling ML strategy: {model_path}")
                settings.strategy_type = "ml"
                settings.ml_model_path = str(model_path)
                # Не переопределяем ml_confidence_threshold, используем значение из .env или по умолчанию (0.6)
            ...
            model_path = pathlib.Path(__file__).parent.parent / "ml_models" / "rf_ETHUSDT_15.pkl"
            if model_path.exists():
                print(f"[config] Found ML model (fallback), enabling ML strategy: {model_path}")
                settings.strategy_type = "ml"
                settings.ml_model_path = str(model_path)
                # Не переопределяем ml_confidence_threshold, используем значение из .env или по умолчанию (0.6)
