...
def analyze_training_logs(log_file: str = None):
    ...
    # Расчет сделок в день (целевой показатель: 1-10 сделок в день)
    try:
    ...
    # Показываем сделки в день (целевой показатель качества)
    if trades_per_day > 0:
        print(f"\n📅 Период обучения: ~{days:.1f} дней")
        print(f"📊 Сделок в день: {trades_per_day:.2f}")
        print(f"🎯 Целевой показатель качества: 1-10 сделок в день")
        if 1 <= trades_per_day <= 10:
            print(f"   ✅ Отлично! Показатель в целевом диапазоне (качественные сделки)")
        elif trades_per_day > 10:
            print(f"   ⚠️  Слишком много сделок. Рекомендуется ужесточить фильтры для повышения качества.")
        else:
            print(f"   ⚠️  Слишком мало сделок. Возможно, фильтры слишком строгие.")
