...
from crypto_env_v17_2_optimized import CryptoTradingEnvV17_2_Optimized
...
class RRMonitoringCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        try:
            # SB3 надежнее отдаёт info через locals['infos']
            env_info = None
            ...
            if isinstance(infos, (list, tuple)) and len(infos) > 0 and isinstance(infos[0], dict):
                ...
                if actions is not None:
                    try:
                        # actions может быть np.ndarray / torch tensor / list
                        a0 = actions[0] if hasattr(actions, '__len__') else actions
                        self.action_history.append(int(a0))
                        if len(self.action_history) > 1000:
                            self.action_history.pop(0)
                    except:
                        pass

                # Логи открытия позиции (из info среды)
                if isinstance(env_info, dict) and env_info.get('last_open') is not None:
                    open_step = env_info.get('last_open_step', None)
                    if open_step is not None and open_step != self._last_open_step:
                        self._last_open_step = open_step
                        o = env_info.get('last_open', {}) or {}
                        try:
                            print(
                                f"[OPEN_CALLBACK] step={o.get('step')} {o.get('side')} "
                                f"entry={float(o.get('entry', 0)):.4f} sl={float(o.get('sl', 0)):.4f} "
                                f"tp1={float(o.get('tp1', 0)):.4f} rr(tp1)={float(o.get('rr_tp1', 0)):.2f}"
                            )
                        except:
                            print(f"[OPEN_CALLBACK] {o}")
                
                # Мониторим RR статистику
                if env_info and 'rr_stats' in env_info:
                ...
                # Мониторим сделки
                if env_info and 'total_trades' in env_info:
...
def train_optimized_model():
    ...
    log_file = os.path.abspath('./logs/v17_2_optimized_v2/train_v17_2_log.csv')
    ...
    def make_train_env():
        ...
        env_params = {
            ...
            'open_log_file': os.path.abspath('./logs/v17_2_optimized_v2/opens_train_v17_2_log.csv'),
        ...
        env = CryptoTradingEnvV17_2_Optimized(**env_params)
        return env
...
def test_model(model, test_df, obs_cols):
    ...
    def make_test_env():
        ...
        env = CryptoTradingEnvV17_2_Optimized(
...
def main():
    ...
    if '--help' in sys.argv or '-h' in sys.argv:
        print("\nИспользование:")
        print("  python train_v17_2_optimized.py          # Интерактивный режим")
        print("  python train_v17_2_optimized.py --new     # Запуск с нуля (без запроса)")
        print("  python train_v17_2_optimized.py --fresh  # То же что --new")
        return
