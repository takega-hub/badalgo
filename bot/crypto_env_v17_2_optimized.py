...
class CryptoTradingEnvV17_2_Optimized(gym.Env):
    """
    ...
    def __init__(self, 
        ...
        """
        super(CryptoTradingEnvV17_2_Optimized, self).__init__()
        ...
        # Логи открытия позиций (отдельный файл, чтобы не ломать анализ сделок)
        ...
        self.trade_history = []
        self.last_open_info: Optional[Dict[str, Any]] = None
        self.last_open_step: Optional[int] = None
    ...
    def _init_open_log_file(self):
        """Инициализация отдельного файла логов открытия позиции"""
        if not self.open_log_file:
            return
        try:
            log_dir = os.path.dirname(self.open_log_file) if os.path.dirname(self.open_log_file) else "."
            os.makedirs(log_dir, exist_ok=True)
    ...
    def _log_open_position(self, side: str, price: float, atr: float):
        """Лог открытия позиции (в консоль + опционально в отдельный CSV)"""
        if not self.log_open_positions:
            return

        try:
            entry = float(self.entry_price)
            sl = float(self.initial_sl)
            tp1 = float(self.tp_prices[0]) if self.tp_prices and len(self.tp_prices) > 0 else 0.0
            tp2 = float(self.tp_prices[1]) if self.tp_prices and len(self.tp_prices) > 1 else 0.0
            tp3 = float(self.tp_prices[2]) if self.tp_prices and len(self.tp_prices) > 2 else 0.0
            ...
            open_info = {
                ...
                "margin_pct": float(getattr(self, "margin_percent", 0.0)),
                "active_margin": float(getattr(self, "active_margin", 0.0)),
                "shares": float(getattr(self, "shares_held", 0.0)),
            }

            self.last_open_info = open_info
            self.last_open_step = int(self.current_step)

            # Консоль
            print(
                f"[OPEN] step={open_info['step']} {open_info['side']} "
                f"entry={open_info['entry']:.4f} sl={open_info['sl']:.4f} "
                f"tp1={open_info['tp1']:.4f} rr(tp1)={open_info['rr_tp1']:.2f} "
                f"atr%={open_info['atr_pct']*100:.2f}% margin%={open_info['margin_pct']*100:.2f}%"
            )
            ...
            if self.open_log_file:
                with open(self.open_log_file, 'a', newline='', encoding='utf-8') as f:
                    writer = csv.writer(f)
                    writer.writerow([
                        open_info["step"], open_info["side"],
                        round(entry, 6), round(sl, 6),
                        round(tp1, 6), round(tp2, 6), round(tp3, 6),
                        f"{rr_tp1:.4f}", f"{atr:.6f}", f"{atr_pct:.6f}",
                        f"{open_info['margin_pct']:.6f}",
                        f"{open_info['active_margin']:.2f}",
                        f"{open_info['shares']:.6f}",
                    ])
        except Exception as e:
            print(f"⚠️ Ошибка логирования открытия позиции: {e}")
    ...
    def _open_long_with_tp_features(self, price: float, atr: float):
        ...
        self._log_open_position("LONG", price=price, atr=atr)
    ...
    def _open_short_with_tp_features(self, price: float, atr: float):
        ...
        self._setup_trade_enhanced(position=-1)
        self._log_open_position("SHORT", price=price, atr=atr)
    ...
    def _get_info(self) -> Dict:
        ...
        return {
            ...
            'last_open': self.last_open_info,
            'last_open_step': self.last_open_step
        }
